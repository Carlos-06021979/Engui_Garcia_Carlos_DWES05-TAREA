...
// Clase Partida, controla el juego completo de ajedrez
  ...
  */
  ...
  public function jugada($origen, $destino)
  {
    ...
    $this->historial[] = serialize([ // Serializamos el estado actual
      ...
      'partidaTerminada' => $this->partidaTerminada // Estado de la partida
    ]);
    ...
    if (!$esEnroque && $piezaOrigen instanceof Peon && !$esCaptura) {
      ...
      if ($difFilas === $direccion && $difCols === 1) {
        ...
        if ($this->ultimoMovimiento && $piezaPosibleCapturada instanceof Peon) {
          ...
          $coordsUMOrig = $this->notacionACoordsLocal($um['origen']); // Coordenadas del origen del último movimiento


          $coordsUMDest = $this->notacionACoordsLocal($um['destino']); // Coordenadas del destino del último movimiento
